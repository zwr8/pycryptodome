...
A *counter block* is exactly as long as the underlying cipher block (e.g. 16 bytes for AES).
It consist of the concatenation of two pieces:

1. a fixed **nonce**, set at initialization.
2. a variable **counter**. It increases by 1 for any subsequent counter block.
   The counter is encoded as big endian.
